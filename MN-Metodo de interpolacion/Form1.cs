...
namespace MN_Metodo_de_interpolacion { //Alan Abiud Castro Cruz - 19100159
    public partial class Form1 : Form {
        private double xl, xu, xr = 0, xrAnterior = 0, ea, criterioError, res;      
        ...
        private void btnCalcular_Click(object sender, EventArgs e) {
            AplicarMetodoInterpolacion();
        }

        private void AplicarMetodoInterpolacion() {
            try {
            ...
            while (bandera && iteraciones < 100) {
                xrAnterior = xr;
                xr = xu - (EvaluarFuncion(xu) * (xl - xu) / (EvaluarFuncion(xl) - EvaluarFuncion(xu)));

                if (iteraciones == 1) ea = Math.Abs((xu - xl) / (xu + xl)) * 100;
                else ea = Math.Abs((xr - xrAnterior) / xr) * 100;
